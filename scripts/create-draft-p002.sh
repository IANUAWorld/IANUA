#!/bin/bash
# Script pour créer le brouillon P002 — Principe IX La Dignité créatrice
# Usage: ./create-draft-p002.sh VOTRE_CLE_ADMIN
  ...
  -d @- << 'ENDJSON'
{
  ...
  "motivation": "La charte adresse la souveraineté humaine dans la décision et l'action. Elle ne protège pas encore explicitement la souveraineté humaine dans la création. Ce nouveau principe nomme trois dangers indissociables — le découragement, la substitution opaque, la dépossession identitaire — et affirme que la créativité humaine brute et sincère a une plus-value irréductible qu'aucune IA ne peut produire. La voix IA sera générée par audit après ratification communautaire — elle ne peut pas être écrite par un humain. Co-réfléchi avec Claude Sonnet 4.6 le 15 mars 2026.",
